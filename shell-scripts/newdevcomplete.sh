...
mysql -u"root" -p"$2" -D"artmis" -e "SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));"
...
su - $SUDO_USER -c "curl -fsSL https://raw.githubusercontent.com/nvm-sh/nvm/master/install.sh | bash"
su - $SUDO_USER -c "export NVM_DIR=\"\$HOME/.nvm\" && source \$NVM_DIR/nvm.sh && nvm install --lts && npm install -g pm2"

echo "Done. phpMyAdmin: http://localhost/phpmyadmin"
echo "MySQL root password: $2"

cp /home/$2/artmis/newdev/artemis.json artemis/config/$2-local.json
sed -i 's/passwddb/'$2'/g' artemisConfig
npm install
npm run build --env=$2-local.json
